---
layout: single
title: '93\. spotkanie - OzCode, Stress, Nauka'
date: 2017-01-17
talks:
- title: OzCode
  speaker_ids:
  - lukasz-kurzyniec
- title: Was Steve Jobs stressed Out?
  speaker_ids:
  - adrian-stefaniak
- title: Learning new language/framework
  speaker_ids:
  - pawel-klimczyk
---

[Archiwalna strona wydarzenia na Meetup](https://www.meetup.com/wrocnet/events/236795560/)

{% include talk.html index=0 %}
* Prezentacja: [OzCode.pdf](/slides/OzCode.zip)
* Wideo: {% include video id="pAJ7YD54ZMA" provider="youtube" %}

{% include talk.html index=1 %}
* Prezentacja: [StressAtwork.pptx](/slides/StressAtwork.zip)
* Materiały: [StressManagement.docx](/slides/StressManagement.zip)
* Wideo: {% include video id="AKMWsB63F0w" provider="youtube" %}

{% include talk.html index=2 %}
* Prezentacja: [pptx](/slides/learning_new_language.zip)
* Wideo: {% include video id="cs1GUajPOVs" provider="youtube" %}