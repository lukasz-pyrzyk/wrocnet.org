...
- **Homepage season calendar**: dates, season, and venue come from [_data/schedule.yml](_data/schedule.yml). Show only current/future dates, highlight the nearest one, link date tiles to Meetup, and keep the mobile list horizontally scrollable so the calendar does not make the homepage unnecessarily long.
